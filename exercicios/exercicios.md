...
// Versão do Docker 26.1.3
$ sudo su
$ sudo apt update
$ sudo apt upgrade
$ sudo apt install apt-transport-https ca-certificates curl software-properties-common // Instale Pacotes Pré-requisitos
$ curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /usr/share/keyrings/docker-archive-keyring.gpg //Adicione a Chave GPG do Docker
$ echo "deb [arch=amd64 signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/ubuntu $(lsb_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
$ sudo apt update
$ sudo apt install docker-ce docker-ce-cli containerd.io // Instale o Docker
$ sudo systemctl status docker // Verifique a instalação
$ sudo usermod -aG docker $USER // Isso permitirá que você execute os comandos Docker sem sudo.
...
Run this if you want to install the docker-ce on Ubuntu 24TLS
...
```sh
$ cd ../node
$ docker build -t devopsdosamba2024/node:devops .
```
Agora iremos rodar a imagem do node, fazendo a ligação dela com o container do Redis.
```sh
$ docker run -d --name node -p 8080:8080 --link redis devopsdosamba2024/node:devops
$ docker ps 
$ docker logs node
```
...
```sh
$ cd ../nginx
$ docker build -t devopsdosamba2024/nginx:devops .
```
Criando o container do nginx a partir da imagem e fazendo a ligação com o container do Node
```sh
$ docker run -d --name nginx -p 80:80 --link node devopsdosamba2024/nginx:devops
$ docker ps
```
...
- Linha 18 = image: devopsdosamba2024/redis:devops
- Linha 37 = image: devopsdosamba2024/node:devops
...
3.90.20.239 //ip rancher-server<br>
54.166.236.245  // ip k8s-1<br>
3.94.209.79    // ip k8s-2<br>
...
$ kubectl apply -f https://raw.githubusercontent.com/traefik/traefik/v2.10/docs/content/reference/dynamic-configuration/kubernetes-crd-rbac.yml // versão nova
